...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for elimination in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, elimination)
            joinedFactorCheck = joinedFactor.unconditionedVariables()
        
            if len(joinedFactorCheck) == 1:
                pass
            else:
                elimFactor = eliminate(joinedFactor, elimination)
                currentFactorsList.append(elimFactor)
        joinedFactor = joinFactors(currentFactorsList)
        normFactor = normalize(joinedFactor)
        return normFactor
