...
PROB_VALS = [PROB_BOTH_TOP, PROB_BOTH_BOTTOM, PROB_ONLY_LEFT_TOP, PROB_ONLY_LEFT_BOTTOM]
...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    "vars = X_POS_VAR, Y_POS_VAR, FOOD_HOUSE_VAR, GHOST_HOUSE_VAR"
    "values = X_POS_VALS, Y_POS_VALS, HOUSE_VALS"
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
	    obsVars.append(obsVar)
            "observations of blue, red, and none"
	...
	edges.append((Y_POS_VAR, var))
    	variableDomainsDict[var] = HOUSE_VALS
    	variableDomainsDict[var] = HOUSE_VALS
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            housePos = {TOP_LEFT_VAL: topLeftPos, BOTTOM_LEFT_VAL: bottomLeftPos, TOP_RIGHT_VAL: topRightPos, BOTTOM_RIGHT_VAL: bottomRightPos}
            
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                foodWallVal = obsPos in gameState.getHouseWalls(housePos[assignment[FOOD_HOUSE_VAR]])
                ghostWallVal = obsPos in gameState.getHouseWalls(housePos[assignment[GHOST_HOUSE_VAR]])
                obsPosType =  assignment[obsVar]
                
                if foodWallVal:
                    if obsPosType == BLUE_OBS_VAL:
			prob = 1 - PROB_FOOD_RED
		    elif obsPosType == RED_OBS_VAL:
			prob = PROB_FOOD_RED
		    else:
			 prob = 0
		elif ghostWallVal:
			...
			prob = PROB_GHOST_RED
	        	...
	        	prob = 0
                elif obsPosType == BLUE_OBS_VAL:
                    prob = 0
                elif obsPosType == RED_OBS_VAL:
                    prob = 0
                elif obsPosType == NO_OBS_VAL:
		    prob = 1
